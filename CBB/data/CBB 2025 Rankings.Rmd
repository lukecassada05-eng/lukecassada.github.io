...
# Step 8: Write CSV
# -------------------------------
write_csv(elo_ranking, "CBB_Elo_2026.csv")
print(elo_ranking)
```
...
game_counts <- games %>%
  ...
  bind_rows(games %>% select(team = away_team)) %>%
  count(team, name = "games_played")

# ===============================
# INITIALIZE OFF / DEF ELO
# ===============================
...
# ===============================
expected_points <- function(off_elo, def_elo) {
  70 + POINT_SCALE * (off_elo - def_elo) / 400
}

# ===============================
# OFFENSE / DEFENSE ELO LOOP
# ===============================
for (iter in 1:NUM_ITER) {
  for (i in 1:nrow(games)) {
    ...
    h_off <- elo$off_elo[elo$team == h]
    h_def <- elo$def_elo[elo$team == h]
    a_off <- elo$off_elo[elo$team == a]
    a_def <- elo$def_elo[elo$team == a]
    
    exp_h_pts <- expected_points(h_off, a_def)
    exp_a_pts <- expected_points(a_off, h_def)
    
    h_delta <- K_FACTOR * log(abs(hp - exp_h_pts) + 1) * sign(hp - exp_h_pts)
    a_delta <- K_FACTOR * log(abs(ap - exp_a_pts) + 1) * sign(ap - exp_a_pts)
    
    # Home offense / Away defense
    elo$off_elo[elo$team == h] <- h_off + h_delta
    elo$def_elo[elo$team == a] <- a_def - h_delta
    
    # Away offense / Home defense
    elo$off_elo[elo$team == a] <- a_off + a_delta
    elo$def_elo[elo$team == h] <- h_def - a_delta
  }
}

# ===============================
# APPLY <8 GAME PENALTY
# ===============================
elo <- elo %>%
  left_join(game_counts, by = "team") %>%
  mutate(
    games_played = ifelse(is.na(games_played), 0, games_played),
    penalty_factor = pmin(games_played / MIN_GAMES, 1),
    off_elo = BASE_ELO + (off_elo - BASE_ELO) * penalty_factor,
    def_elo = BASE_ELO + (def_elo - BASE_ELO) * penalty_factor
  )
...
elo_final <- elo %>%
  mutate(
    off_elo = round(off_elo, 1),
    def_elo = round(def_elo, 1)
  )

off_rank <- elo_final %>%
  arrange(desc(off_elo)) %>%
  mutate(off_rank = row_number()) %>%
  select(team, off_rank, off_elo)

def_rank <- elo_final %>%
  arrange(desc(def_elo)) %>%
  mutate(def_rank = row_number()) %>%
  select(team, def_rank, def_elo)

# ===============================
# OUTPUT
# ===============================
elo_out <- off_rank %>%
  left_join(def_rank, by = "team") %>%
  arrange(off_rank)

write_csv(elo_out, "CBB_Offense_Defense_Elo_2026.csv")
...
# ===============================
elo <- read_csv(
  "CBB_Offense_Defense_Elo_2026.csv",
  show_col_types = FALSE
) %>%
  select(team, off_elo, def_elo)
...
games <- future_games %>%
  ...
  rename(home_off = off_elo, home_def = def_elo) %>%
  left_join(elo, by = c("away_team" = "team")) %>%
  rename(away_off = off_elo, away_def = def_elo) %>%
  mutate(
    across(c(home_off, home_def, away_off, away_def),
           ~ ifelse(is.na(.x), BASE_ELO, .x))
  )
...
games <- games %>%
  mutate(
    home_off_adj = ifelse(neutral_site, home_off, home_off + HCA)
  )
...
games <- games %>%
  mutate(
    exp_home_pts = 70 + POINT_SCALE * (home_off_adj - away_def) / 400,
    exp_away_pts = 70 + POINT_SCALE * (away_off - home_def) / 400,
    projected_total = round(exp_home_pts + exp_away_pts, 1)
  )
...
games <- games %>%
  mutate(
    net_elo = (home_off_adj - away_def) - (away_off - home_def),

    home_win_pct = 1 / (1 + 10^(-net_elo / 400)),
    away_win_pct = 1 - home_win_pct,

    pick_confidence = round(abs(home_win_pct - 0.5) * 200, 1),
    spread_confidence = round(pmin(100, abs(net_elo) / 300 * 100), 1),

    projected_spread = round(net_elo / SPREAD_SCALE, 1)
  )
...
write_csv(
  predictions,
  "CBB_OffDef_Elo_Future_Game_Predictions.csv"
)
...
# ===============================
elo <- read_csv("CBB_Elo_2026.csv", show_col_types = FALSE) %>%
  # split record column "16-3" into numeric wins/losses
  mutate(
...
# WRITE CSV
# ===============================
write_csv(projected_records, "CBB_Projected_Final_Records_2026.csv")
...
# WRITE CSV
# ===============================
write_csv(final_df, "CBB_Projected_Records_SOS_2026.csv")
print(final_df)
```
...
# -------------------------------
write_csv(conf_summary, "CBB_Conference_Elo_2026.csv")
print(conf_summary)
```
...
write_csv(
  predictions,
  "CBB_Elo_Future_Game_Predictions.csv"
)
