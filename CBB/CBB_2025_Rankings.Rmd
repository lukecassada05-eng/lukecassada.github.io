...
cbb_clean <- cbb_sel %>%
  ...
  filter(!is.na(home_pts), !is.na(away_pts), home_pts != away_pts) %>%
  mutate(
    winner = if_else(home_pts > away_pts, home_short_display_name, away_short_display_name),
    loser  = if_else(home_pts < away_pts, home_short_display_name, away_short_display_name),
...
# Step 4: Build team -> conference lookup
team_conf_lookup <- cbb_all %>%
  filter(groups_is_conference == TRUE) %>%
  select(team = home_short_display_name, conference = groups_name) %>%
  distinct()

# Attach conferences to winner and loser
cbb_clean <- cbb_clean %>%
  left_join(team_conf_lookup %>% rename(winner = team, winner_conf = conference), by = "winner") %>%
  left_join(team_conf_lookup %>% rename(loser = team, loser_conf = conference), by = "loser") %>%
  mutate(
...
cbb_clean <- cbb_sel %>%
  ...
  # remove unfinished games, ties, and bad rows
  filter(
    !is.na(home_pts),
    !is.na(away_pts),
    home_pts != away_pts
...
team_conf_lookup <- cbb_all %>%
  filter(groups_is_conference == TRUE) %>%
  select(team = home_short_display_name,
         conference_id = groups_id,
         conference_name = groups_name) %>%
  distinct()

# Map winner and loser teams to their conferences
cbb_clean <- cbb_clean %>%
  left_join(team_conf_lookup %>%
              rename(winner = team,
                     winner_conf_id = conference_id,
                     winner_conf = conference_name),
            by = "winner") %>%
  left_join(team_conf_lookup %>%
              rename(loser = team,
                     loser_conf_id = conference_id,
...
# Build team -> conference lookup from ALL teams in the season
team_conf_lookup <- cbb_all %>%
  filter(groups_is_conference == TRUE) %>%
  select(team = home_short_display_name, conference = groups_name) %>%
  distinct()

# Attach conferences to winner and loser safely
cbb_clean <- cbb_clean %>%
  left_join(team_conf_lookup %>% rename(winner = team, winner_conf = conference), by = "winner") %>%
  left_join(team_conf_lookup %>% rename(loser = team, loser_conf = conference), by = "loser") %>%
  mutate(
